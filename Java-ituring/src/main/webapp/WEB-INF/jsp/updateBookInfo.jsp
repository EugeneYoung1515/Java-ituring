                ...
                <div class="row g-20">
                    <div class="col-md-4">
                        <div class="book-img">
                            ...
                            <div style="width:120px;margin: 10px auto">
                                <input id="upload-book-image" name="upload-book-image" type="file" />
                            </div>
                            <div id="book-fav-vote" class="book-actions">
                                ...
                                <script>
                                    $(document).ready(function () {
                                        $("#upload-book-image").kendoUpload({
                                            multiple: false,
                                            showFileList: false,
                                            async: {
                                                saveUrl: "/upload/image",
                                                autoUpload: true
                                            },
                                            success: function (e) {
                                                $.post("/book/setimage", { key: e.response.key }, function (data) {
                                                    $(".col-md-4").html(data);
                                                });
                                            },
                                            upload: function (e) {
                    ...
                    <div class="col-md-8">
                        <div class="book-head-upper">
                            <div class="book-series">${book.series.seriesName}
                                <select name="seriesId">
                                    <option>选择其他系列</option>
                                    ...
                                    <table>
                                        <tr>
                                            <td>
                                                ...
                                                <option value="${originalAuthor.originalAuthorId}">${originalAuthor.authorName}</option>
                                                    </c:forEach>
                                                </select>
                                            </td>
                                            <td><input type="button" value="删除作者" onclick="deleteAuthor(this)"/></td>
                                        </tr>
                                    </table>
                                </span>
                                <input type="button" value="添加新作者" onclick="newAuthor()"/>
                                <span>
                                    <table>
                                        <tr>
                                            <td><select>
                                                <c:forEach var="translatorAuthor" items="${book.translatorAuthors}" varStatus="stat">
                                                <option value="${translatorAuthor.authorName}">${translatorAuthor.authorName}</option>
                                                </c:forEach></select>
                                            </td>
                                            <td>
                                                <input type="button" value="删除译者" onclick="deleteAuthor(this)"/>
                                            </td>
                                        </tr>
                                    </table>
                                        (译者)
                                </span>
                                <input type="button" value="添加新译者" onclick="newAuthor2()"/>
                            </div>
                            <script type="text/javascript">
                                function newAuthor() {
                                    var bookAuthor = $(".book-author");
                                    var bookAuthorDom = bookAuthor.get(0);
                                    var a = bookAuthorDom.children[0];
                                    var table = document.createElement('table');
                                    a.appendChild(table);
                                    table.innerHTML='<tr><td><select><c:forEach var="originalAuthor" items="${book.originalAuthors}" varStatus="stat"><option value="${originalAuthor.authorName}">${originalAuthor.authorName}</option></c:forEach></select></td><td><input type="button" value="删除作者" onclick="deleteAuthor(this)"/></td></tr>';
                                }
                                function newAuthor2() {
                                    var bookAuthor = $(".book-author");
                                    var bookAuthorDom = bookAuthor.get(0);
                                    var a = bookAuthorDom.children[2];
                                    var table = document.createElement('table');
                                    a.appendChild(table);
                                    table.innerHTML='<tr><td><select><c:forEach var="translatorAuthor" items="${book.translatorAuthors}" varStatus="stat"><option value="${translatorAuthor.authorName}">${translatorAuthor.authorName}</option></c:forEach></select></td><td><input type="button" value="删除译者" onclick="deleteAuthor(this)"/></td></tr>';
                                }
                            ...
                            <div class="book-status">
                                ...
                                <span class="tags">
                                    <c:forEach var="booktag" items="${book.booktags}"><!--这几换成选项比较好-->
                                        <a href="/tag/${booktag.booktagId}" class="post-tag"><input type="text" name ="booktag.booktagName" value="${booktag.booktagName}"></a>
                                    </c:forEach>
                                    </span>
                            </div>
                            <div class="book-intro readmore">
                                <textarea style="width:100%;height:200px"  name="bookIntro">${book.bookIntro}</textarea>
                            </div>
        ...
        <div class="col-md-3 pull-right side">
            ...
            <div class="book-approaches">
                <dl>
                    <dt>电子书</dt>
                    <dd>
                        <span class="price">￥<input type="text" name ="ebookPrice" value="${book.ebookPrice}"></span>
                    </dd>
                </dl>
                ...
                <dl>
                    ...
                    <dd>
                            <span class="price">
                                ￥<input type="text" name ="paperSalePrice" value="${book.paperSalePrice}">
                            </span>
                        <s>￥<input type="text" name ="paperOriginalPrice" value="${book.paperOriginalPrice}"></s>
                    </dd>
                </dl>
                <div class="buy-btns">
                ...
                <div class="where-to-buy">
                    <h4>其他购买方式？</h4>
                    <ul>
                        <li><a href="https://amzn.to/2PQZA0K" target="_blank">到亚马逊购买</a></li>

                        <li><a href="http://union.dangdang.com/transfer.php?from=P-296766&ad_type=10&sys_id=1&backurl=http://product.dangdang.com/25576286.html" target="_blank">到当当购买</a></li>

                        <li><a href="http://click.union.jd.com/JdClick/?unionId=50375&to=https://item.jd.com/12451331.html?dist=jd" target="_blank">到京东购买</a></li>

                        <li><a href="http://www.china-pub.com/log/cpsclick.aspx?id=27&url=http://product.china-pub.com/8053525" target="_blank">到互动购买</a></li>
            ...
            <div class="block">
                ...
                <div class="block-body">
                    <ul class="publish-info">
                        <li><strong>执行编辑</strong>关于本书的内容有任何问题，请联系 <a href="/message/chat/160851?Name=%E6%88%91%E7%9A%84%E7%AC%AC%E4%B8%80%E6%9C%AC%E7%AE%97%E6%B3%95%E4%B9%A6"><input type="text" name ="publishInfo.questionContact" value="${book.publishInfo.questionContact}"></a></li>
                        <li><strong>出版日期</strong>
                            <fmt:formatDate pattern="yyyy-MM-dd"
                                            value="${book.publishInfo.bookDate}" />
                        </li>                        <li><strong>书　　号</strong><input type="text" name ="publishInfo.bookNumber" value="${book.publishInfo.bookNumber}"></li>
                        <li><strong>页　　数</strong><input type="text" name ="publishInfo.pageNum" value="${book.publishInfo.pageNum}"></li>
                        <li><strong>印刷方式</strong><input type="text" name ="publishInfo.print" value="${book.publishInfo.print}"></li>
                        <li><strong>开　　本</strong><input type="text" name ="publishInfo.bookSize" value="${book.publishInfo.bookSize}"></li>
                        <li>
                            <strong>原书名</strong><input type="text" name ="publishInfo.originalTitle" value="${book.publishInfo.originalTitle}">
                        </li>
                        <li>
                            <strong>原书号</strong><input type="text" name ="publishInfo.originalBookNumber" value="${book.publishInfo.originalBookNumber}">
                        </li>
                        <li><strong>原书链接</strong><input type="text" name ="publishInfo.amazonLink" value="${book.publishInfo.amazonLink}"></li>
                    </ul>
                </div>
            </div>
    ...
    <c:forEach var="translatorAuthor" items="${sBook.translatorAuthors}">
        ...
        <div class="col-md-9 main tag-at-top">
            ...
            <div class="block">
                ...
                <div class="block-body">
                    <div class="intro">
                        <textarea style="width:100%;height:200px" name="feature">${book.feature}</textarea
                    </div>
                </div>
            </div>
...
<c:forEach var="originalAuthor" items="${book.originalAuthors}">
    ...
    <textarea style="width:100%;height:200px"  name="originalAuthorIntro">${originalAuthor.originalAuthorIntro}</textarea></br></br>
</c:forEach>
            ...
            <div class="block block-books block-books-grid">
                ...
                <div class="block-header">
                    <h4>大家也喜欢<select name="alsoLikeId">
                        <option>选择其他大家也喜欢</option>
                        <c:forEach var="alsoLike" items="${allAlsoLike}">
                            <option value="${alsoLike.alsoLikeId}">${alsoLike.alsoLikeName}</option>
                        </c:forEach>
                    </select>
                    </h4>
                </div>
